...
static inline float        GetFloatValue(const char *str,const char **next)
{
	...
	*dst = 0;
	...
	{
		if ( stricmp(dest,"fltmax") == 0 || stricmp(dest,"fmax") == 0 ||  stricmp(dest,"FLT_MAX") == 0 || stricmp(dest,"INF") == 0 )
		{
			ret = FLT_MAX;
		}
		else if ( stricmp(dest,"fltmin") == 0 || stricmp(dest,"fmin") == 0 || stricmp(dest,"FLT_MIN") == 0 || stricmp(dest,"-INF") == 0 )
		{
			ret = FLT_MIN;
		}
	}
